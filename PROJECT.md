...
webcam-guard\                  # 레포. 배포 위치는 어디든 상관없다
  ...
  ├─ PROJECT.md                # 이 문서
  ├─ CHANGELOG.md              # 무엇이 왜 바뀌었는지 (뒤집힌 결정 포함)
...
- **강조색은 한 종류뿐이다**: 녹화 표시등, 선택된 항목, 타임라인, 용량 게이지, 로그북의 얼굴 표시. 그 외에는 무채색.
- 모바일 우선. `prefers-reduced-motion` 존중, 키보드 포커스 링 있음.
- **자산은 `static/`에 내장**되어 있다(`tools/vendor.py` 생성물). 파일이 없으면 hls.js는 CDN으로, 폰트는 시스템 폰트로 자연스럽게 내려간다. iOS Safari는 네이티브 HLS라 hls.js 자체가 필요 없다.

### 시그니처 요소 = 24시간 타임라인 바 (`.strip`)

하루치를 가로 막대 하나에 뿌린다. **함부로 없애지 말 것** — 상시녹화라는 이 프로젝트의 성격을 그대로 드러내는 요소다.

로그북이 들어오면서 **두 레인**이 됐다.
...
**Why 농도로 구분하나**: 위 규칙이 새 색 추가를 금지한다. 세 가지를 구분해야 하는데 색을 늘리면 그 규칙이 무너지므로, 같은 색조의 농도 차로 위계를 만들었다. 농도만으로는 애매한 구분(움직임 vs 얼굴)은 **목록 쪽에서 글자로** 확실히 말한다("얼굴 2" / "움직임"). 색은 훑어볼 때, 글자는 확인할 때 쓰는 것이다.

### 로그북 목록 (`.ev`)

`시각 · 종류 · 지속시간` 세 칸. 누르면 그 시각으로 이동해 재생된다(`playClipAt` → `playClip(..., offset)`).

**낡은 렌더를 버리는 장치가 있다.** `loadEvents()`는 비동기로 받아온 뒤 `.strip`에 마커를 덧붙이는데, 그 사이 다른 렌더가 스트립을 비우면 두 벌이 겹쳐 쌓인다(실제로 마커 28개 vs 목록 14개로 재현했다). `renderSeq`를 비교해 자기 차례가 지났으면 그리지 않는다. **이 검사를 지우지 말 것.**
...
| 디스크 | 720p/15fps/1500k 기준 **하루 15.4GB**(실측). `--max-gb`를 디스크 여유보다 작게 잡지 않으면 C드라이브가 차서 Windows가 멈춘다 |
| 동시 시청 | ThreadingHTTPServer. 라이브 재생목록은 12스레드로 두들겨봤지만(5-11절), 여러 명이 **실제로 영상을 재생하는** 대역폭 부하는 안 봤다 |
| 로그북 CPU | 2초 세그먼트당 모션 50ms + 얼굴 78ms = **코어 하나의 약 6%**(실측). 부담되면 `--no-faces`가 절반 이하로 줄인다 |
| 얼굴 감지 의존성 | OpenCV가 있어야 동작한다. 없으면 조용히 모션만 기록한다 — **얼굴 사건이 안 보인다고 고장이 아닐 수 있다.** `/api/status`의 `faces_available`로 확인할 것 |
| 로그북 오탐 | 카메라 자동노출이 안정화되는 **기동 직후 첫 세그먼트**는 화면 전체가 변해 사건으로 기록된다. 무해하지만 하루에 재시작 수만큼 쌓인다 |

---

## 9. 다음에 할 만한 작업 (우선순위 제안)

1. **장시간 안정성 관찰** — 2절의 미검증 1번. 하루 이상 켜두고 janitor 삭제와 워치독 재시작을 실제로 보는 것. **다른 무엇보다 이게 먼저다** — 아직 수 분 단위로만 돌려봤다.
2. **로그북 실사용 확인** — 하루치가 쌓인 뒤 `events/`를 열어 실제로 쓸 만한지 볼 것. 빈 방인데 사건이 잔뜩이면 `motion_threshold`를 올리고, 사람이 지나갔는데 비었으면 내린다. `tools/calibrate.py`가 그 판단을 도와준다. **감도는 방마다 다르므로 기본값 1.0%를 정답으로 취급하지 말 것.**
3. **로그북 보존 정책** — 지금 `events/*.jsonl`은 **아무도 지우지 않는다.** 하루 수 KB라 몇 년을 둬도 문제없지만, 클립은 10일 뒤 사라지는데 그 시간대를 가리키는 기록만 남는다. 뷰어는 이미 "클립이 아직 없어"를 처리하지만, 지난 기록을 정리할지 남길지는 정해야 한다.
...
- **이벤트 알림(푸시/텔레그램/이메일)** — 오탐이 섞인 알림은 며칠이면 무시하게 되고, 그때부터 알림도 로그북도 신뢰를 잃는다. 적어두기만 하면 사용자가 필요할 때 확인하고 틀린 항목은 그냥 넘기면 된다. **요청이 오면 이 트레이드오프를 먼저 설명할 것.**
- **신원을 식별하는 얼굴 인식** — 지금은 얼굴이 "있다/없다"만 본다. 누구인지 저장하기 시작하면 이 프로그램이 다루는 데이터의 성격이 완전히 달라진다(생체정보). 기술적 난이도가 아니라 성격의 문제다.
- **오디오** — 5-5절.
...
- 기능을 추가할 때는 **의존성 0개 원칙**을 지킬 것. pip 패키지를 넣는 순간 사용자의 설치 난이도가 크게 올라간다. (`tools/`는 개발 전용이라 이 원칙 밖이지만, 거기서도 표준 라이브러리만 썼다. OpenCV는 **있으면 쓰고 없으면 건너뛰는** 방식이라 원칙을 깨지 않는다 — 5-14절.)
- **감지 로직을 건드렸으면 실제 카메라로 재보정할 것.** `tools/calibrate.py`를 빈 방과 사람이 있는 상태로 두 번 돌린다. 합성 영상만으로는 방의 노이즈 바닥을 알 수 없다 — 이 프로젝트에서 실제로 두 번(지표 자체, 얼굴 게이팅) 설계가 뒤집힌 지점이다.

### 이 프로젝트에서 반복된 실패 패턴

같은 함정을 다시 밟지 않도록 적어둔다. 셋 다 **"돌려보기 전까지는 맞다고 믿었던 것"**이다.

| 패턴 | 실제 사례 |
|---|---|
| **합성 테스트만으로 판단** | 모션 임계값 6.0. 합성 패턴에서는 그럴듯했지만 실측하면 사람이 안 잡혔다 (5-13절) |
| **CPU를 아끼려다 요구사항을 놓침** | 얼굴을 모션 뒤에 게이팅 → 가만히 있는 사람을 통째로 놓쳤다 (5-14절) |
| **고친 위치가 너무 좁음** | UTF-8 재설정을 `main()`에 넣었는데 스레드·도구는 `main()`을 안 거쳐 그대로 죽었다. 모듈 로드 시점이 맞았다 |

공통 교훈: **예외가 나지 않는 실패가 제일 위험하다.** 반쪽짜리 재생목록(5-11절), 아무것도 기록 안 되는 로그북(5-13절)은 둘 다 조용히 틀렸고, 테스트를 먼저 쓴 덕분에 잡혔다.
